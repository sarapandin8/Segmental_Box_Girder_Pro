...
def test_m3d_schema_version_is_updated():
    ...
    assert 'PROJECT_SCHEMA_VERSION = "0.4.82-commercial-psloss22-time-dependent-loss-tabs"' in validation_src
...
def test_psloss3_adopted_tendon_and_formula_readiness_register():
    ...
    assert "PSLOSS.22 calculation-readiness snapshot" in src
...
def test_psloss14_header_and_next_step_consistency_cleanup():
    ...
    assert "4.5 Time-Dependent Losses component tabs are active" in src
    assert "4.5 Time-Dependent Losses now groups creep, shrinkage, relaxation" in src
    assert "segmental_box_girder_psloss22_summary.md" in src
...
def test_psloss21_relaxation_source_model_and_gated_preview():
    ...
    assert "AASHTO refined R1/R2 preview — Recommended" in src
