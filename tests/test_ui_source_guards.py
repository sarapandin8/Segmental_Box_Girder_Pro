...
def test_fea5a_dashboard_status_tracks_three_stage_import_engine():
    src = _src()
    assert "Three-stage import ready" in src
    assert "Complete Section 5 source import before downstream envelope design" in src
    assert "cross_stage_station_consistency" in src


def test_m3d_schema_version_is_updated():
    validation_src = VALIDATION_SOURCE.read_text(encoding="utf-8")
    assert 'PROJECT_SCHEMA_VERSION = "0.5.0-commercial-fea5a-three-stage-force-import-envelope-hub"' in validation_src
...
def test_fea5a_three_stage_import_and_component_envelope_hub():
    ...
    assert "Raw candidate rows are preserved" in src
    assert "Do not treat the compact row as a simultaneous force vector" in src
